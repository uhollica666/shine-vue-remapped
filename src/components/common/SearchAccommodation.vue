  ...
  <div>
    <div class="accommodation-element mt-5">
      <h4 class="text-capitalize">
        Your Searched for Properties between Nu.
        <u class="fs-4">{{ $route.params.category }}</u>
      </h4>
    </div>
    <div class="row mb-3">
      ...
      <template v-for="accommodation in filterProducts1">
        ...
        <div v-if="!accommodation" :key="accommodation.price">
          <h5>No Accommodations Available at this Price Range</h5>
        </div>
      </template>
...
export default {
  ...
  methods: {
    filteredProducts() {
      return this.Accommodations.filter((product) => product.price > 0.0);
    },
    filteredProductsA() {
      return this.Hotels.filter((product) => product.price > 0.0);
    },
    filteredProducts1() {
      return this.Accommodations.filter(
        (product) => product.price > 0.0 && product.price <= 5000.0
      );
    },
    filteredProductsB() {
    ...
    filteredProducts2() {
      return this.Accommodations.filter(
        (product) => product.price > 5000.0 && product.price <= 10000.0
      );
    },
    filteredProductsC() {
      return this.Hotels.filter(
    ...
    filteredProducts3() {
      return this.Accommodations.filter(
        (product) => product.price > 10000.0 && product.price <= 15000.0
      );
    },
    filteredProductsD() {
      return this.Hotels.filter(
    ...
    filteredProducts4() {
      return this.Accommodations.filter((product) => product.price > 15000.0);
    },
    filteredProductsE() {
      return this.Hotels.filter((product) => product.price > 15000.0);
    },
  },
  computed: {
    filterProducts() {
